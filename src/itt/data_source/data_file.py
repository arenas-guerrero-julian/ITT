...
def _read_tabular_view(rml_rule):
    return duckdb.query(rml_rule['logical_source_value']).pl()
...
def _read_csv(rml_rule, references, file_source_type):
    ...
    try:
        return pl.read_csv(rml_rule['logical_source_value'],
                             separator=delimiter,
                             encoding='utf-8',
                             columns=references)
    except:
...
def _read_parquet(rml_rule, references):
    return pl.from_pandas(pd.read_parquet(rml_rule['logical_source_value'], engine='pyarrow', columns=references))


def _read_feather(rml_rule, references):
    return pl.from_pandas(pd.read_feather(rml_rule['logical_source_value'], use_threads=False, columns=references))


def _read_orc(rml_rule, references):
    return pl.from_pandas(pd.read_orc(rml_rule['logical_source_value'], encoding='utf-8', columns=references))


def _read_stata(rml_rule, references):
    return pl.from_pandas(pd.read_stata(rml_rule['logical_source_value'],
                         columns=references,
                         convert_dates=False,
...
def _read_sas(rml_rule):
    return pl.from_pandas(pd.read_sas(rml_rule['logical_source_value'], encoding='utf-8'))
...
def _read_excel(rml_rule, references):
    return pl.from_pandas(pd.read_excel(rml_rule['logical_source_value'],
                         sheet_name=0,
                         engine='openpyxl',
                         usecols=references,
                         dtype=str,
