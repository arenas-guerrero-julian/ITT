...
def _materialize_fnml_template(data, template):
    ...
    data = data.with_columns(pl.lit('').alias('aux_fnml_template_data'))

    for reference in references:
        data = data.with_columns(pl.col(reference).alias('reference_results'))

        splitted_template = template.split('{' + reference + '}')
        data = data.with_columns(pl.concat_str(pl.col('aux_fnml_template_data') + splitted_template[0] + pl.col('reference_results')).alias('aux_fnml_template_data'))
        template = str('{' + reference + '}').join(splitted_template[1:])
    if template:
        # add what remains in the template after the last reference
        data = data.with_columns(pl.concat_str(pl.col('aux_fnml_template_data') + template).alias('aux_fnml_template_data'))
...
def execute_fnml(data, fnml_df, fnml_execution, config):
    ...
    data = data.with_columns([
        pl.col(data.columns[0]),
        pl.Series(fnml_execution, exec_res, dtype=pl.Utf8),
    ])
